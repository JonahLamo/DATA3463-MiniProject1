...
df['Units_Q1_2024'] = pd.to_numeric(df['Units_Q1_2024'], errors='coerce')
df['Units_Q1_2023'] = pd.to_numeric(df['Units_Q1_2023'], errors='coerce')
...
# Done here so that the error shows that the problem was in the 'Units_Q1_2024' column rather than just saying there was an error in the 'YoY%_Zoning' column
...
df['YoY%_Zoning'] = round((df['Units_Q1_2024'] - df['Units_Q1_2023']) / df['Units_Q1_2023'] * 100, 1)
# print(df) # Check to make sure the new column was created correctly, and that the values are correct
